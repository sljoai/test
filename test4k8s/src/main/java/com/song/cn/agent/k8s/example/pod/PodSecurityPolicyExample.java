    ...
    public static void main(String args[]) {
        try (final KubernetesClient client = new DefaultKubernetesClient()) {
            ...
            try (final InputStream localYamlStream = PodSecurityPolicyExample.class.getResourceAsStream(localYamlToCreate)) {
                final PodSecurityPolicy podSecurityPolicy = client.policy()
                        .podSecurityPolicies()
                        .load(localYamlStream)
                        .get();
                client.policy()
                        .podSecurityPolicies()
                        .withName(podSecurityPolicy.getMetadata()
                                .getName())
                        .delete();
                client.policy()
                        .podSecurityPolicies()
                        .withName(podSecurityPolicy.getMetadata().getName())
